											...
											<tr className='bg-[#FFFFFF] hover:bg-[#f1f1f16c] flex-col items-start xl:flex-row flex justify-between w-full py-[10px]'>
												...
												<td
													onClick={() => {
														document.getElementById('categoryModal').showModal()
														setPostOrPut(
															animals?.docs.filter(i => i.id == item.id)[0].id
														)
														setCategoryInp({
															name: animals?.docs
									...
									<tbody className='rounded-[30px] w-full'>
										{categories?.docs.sort().map((item, index) => (
											<tr className='bg-[#FFFFFF] hover:bg-[#f1f1f16c] flex-col items-start 2xl:flex-row flex justify-between w-full py-[10px]'>
												<th className='w-[65px] hidden xl:block text-center'>
													{index + 1}
												</th>
												...
												<td
													onClick={() => {
														document.getElementById('animalModal').showModal()
														setPostOrPut(
															categories?.docs.filter(i => i.id == item.id)[0]
																.id
														)
														setAnimalsInp({
															title: categories?.docs
																.filter(i => i.id == item.id)[0]
																.data().title,
															desc: categories?.docs
																.filter(i => i.id == item.id)[0]
																.data().desc,
															image: categories?.docs
																.filter(i => i.id == item.id)[0]
																.data().image,
														})
													}}
			...
			<dialog id='animalModal' className='modal'>
				<div className='modal-box'>
					...
					<div className='w-full flex flex-col gap-2 mt-[15px]'>
						<input
							onChange={e =>
								setAnimalsInp(prev => ({ ...prev, title: e.target.value }))
							}
						...
						<input
							onChange={e =>
								setAnimalsInp(prev => ({ ...prev, desc: e.target.value }))
							}
						...
						<input
							onChange={e =>
								setAnimalsInp(prev => ({ ...prev, image: e.target.value }))
							}
